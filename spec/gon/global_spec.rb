...
describe Gon::Global do
  ...
  context 'with jbuilder and rabl' do
    ...
    it 'works fine with rabl' do
      ...
      Gon.global.rabl template: template, controller: controller
      expect(Gon.global.objects.length).to eq(2)
    end

    it 'works fine with jbuilder' do
      template = 'spec/test_data/sample.json.jbuilder'
      Gon.global.jbuilder template: template, controller: controller
      expect(Gon.global.objects.length).to eq(2)
    end
